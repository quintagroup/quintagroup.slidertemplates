$.fn.qgCarousel = function () {
  ...
  function widthCalculate () {
    ...
    if ($(window).width() >= 768) {
      ...
      item.find(".item-visual").css({'height':imageWidth})
      ...
      item.find(".image-block").css({'width':imgItem,'height':imgItem});
  ...
  function next() {
    ...
    if ($(window).width() >= 768) {
      ...
      if(itemActive.is(':last-child')) {
        ...
        base.find('.qg-item:not(.active) .image-block').css({'width':imgItem,'height':imgItem});
        base.find('.qg-item.active .image-block').css({'width':imageWidth,'height':imageWidth});
        setTimeout(function(){
